#!/usr/bin/env python3
"""
AI Tagging for Watch Pages
Tags watch images using Gemini AI with structured watch attributes
"""
...
def tag_watch_pages(csv_path, output_json, output_csv, max_videos=None, batch_size=50):
    """Main function to tag watch pages"""
    
    print("🏷️ AI Watch Tagging System")
    print("=" * 50)
        ...
        for batch_num in range(total_batches):
            ...
            for i, video in enumerate(batch):
                ...
                future = executor.submit(
                    tag_single_video,
                    model, video, video_num, len(videos), prompt
                )
    ...
    try:
        ...
        print(f"✅ Saved JSON: {output_json} ({len(tagged_videos)} videos)")
    except Exception as e:
        print(f"❌ Error saving JSON: {e}")
        return False
    
    # Save CSV
    if save_tagged_csv(tagged_videos, output_csv):
        print(f"✅ Saved CSV: {output_csv} ({len(tagged_videos)} videos)")
    else:
        print(f"⚠️ JSON saved but CSV failed")
    ...
    print(f"  - {output_json}")
    print(f"  - {output_csv}")
    print(f"\n🌐 Next step: Generate gallery with 'python3 generate_watch_pages_gallery.py'")
...
def main():
    ...
    output_csv = f"{csv_name}_tagged.csv"
    ...
    # Run tagging
    success = tag_watch_pages(csv_path, output_json, output_csv, max_videos=max_videos)
